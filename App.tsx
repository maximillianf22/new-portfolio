...
const projects: ProjectItem[] = [
  {
    id: '1',
    title: 'Neon Nexus',
    description: 'A cyberpunk-inspired DeFi dashboard with real-time WebSocket data visualization.',
  ...
  },
  {
    id: '2',
    title: 'Vocalize AI',
    description: 'Text-to-speech platform utilizing the latest Gemini Audio models for realistic voice synthesis.',
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen text-white selection:bg-purple-500/30 font-sans">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-6 pointer-events-none">
        <div className="pointer-events-auto bg-neutral-900/80 backdrop-blur-md border border-neutral-800 rounded-full pl-6 pr-2 py-2 flex items-center gap-6 shadow-2xl">
           ...
           <div className="hidden md:flex gap-4 text-sm font-medium text-neutral-400">
             <a href="#about" className="hover:text-white transition-colors">{lang === 'en' ? 'About' : 'Sobre Mí'}</a>
             <a href="#skills" className="hover:text-white transition-colors">{lang === 'en' ? 'Skills' : 'Habilidades'}</a>
             <a href="#projects" className="hover:text-white transition-colors">{lang === 'en' ? 'Projects' : 'Proyectos'}</a>
             <a href="#contact" className="hover:text-white transition-colors">{lang === 'en' ? 'Contact' : 'Contacto'}</a>
           </div>
           ...
           >
             <Download size={14} />
             <span className="hidden sm:inline">{lang === 'en' ? 'CV' : 'CV'}</span>
           </button>
      ...
      <section id="projects" className="py-24 max-w-7xl mx-auto px-4">
        <BlurFade>
            <h2 className="text-3xl font-bold mb-12 text-white/90">{lang === 'en' ? 'Selected Projects' : 'Proyectos Destacados'}</h2>
        </BlurFade>
