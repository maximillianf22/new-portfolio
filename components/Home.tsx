...
export const Home: React.FC = () => {
  ...
  const employmentProjects = projects.filter(p => p.category !== 'contract');
  const contractProjects = projects.filter(p => p.category === 'contract');

  const [employmentVisible, setEmploymentVisible] = useState<number>(2);
  const [contractVisible, setContractVisible] = useState<number>(2);
  ...
  return (
    <div className="bg-[#0a0a0a] min-h-screen text-white selection:bg-purple-500/30 font-sans">
      ...
      <main className="relative z-10">
        ...
        {/* 4. TESTIMONIALS - Validación social temprana */}
        {content.testimonials && content.testimonials.length > 0 && (
        ...
        {/* 5. SELECTED PROJECTS - Trabajo destacado con demos */}
        <section id="projects" className="py-24 max-w-7xl mx-auto px-4 scroll-mt-24">
          ...
          {/* Employment Projects - Carousel 2 by 2 */}
          <ProjectsCarousel
            title={lang === "en" ? "Employment" : "Experiencia laboral"}
            projects={employmentProjects}
            lang={lang}
          />

          {/* Contracting Projects */}
          {projects.some(p => p.category === 'contract') && (
            <>
              <BlurFade>
                <h3 className="text-xl font-semibold text-white/80 mb-6 mt-12">
                  {lang === "en" ? "Consulting & Contract" : "Consultoría y Contratos"}
                </h3>
              </BlurFade>
              <ProjectsCarousel
                title={lang === "en" ? "Consulting & Contract" : "Consultoría y Contratos"}
        ...
        {/* 6. AI PLAYGROUND - Diferenciador único */}
        <section id="ai-playground" className="scroll-mt-24">
          <AIPlayground lang={lang} />
        </section>

        {/* 7. EXPERIENCE - Historial completo para quien quiera profundizar */}
        <section id="experience" className="scroll-mt-24">
          <Experience experience={content.experience} lang={lang} />
        </section>

        {/* 8. CONSULTING - Experiencia adicional */}
        {content.consulting && content.consulting.length > 0 && (
          <section id="consulting" className="scroll-mt-24">
            <Experience
              experience={content.consulting}
              lang={lang}
              title={lang === 'en' ? 'Consulting & Contract' : 'Consultoría y Contratos'}
              dateField="duration"
            />
        ...
        {/* 9. BLOG - Contenido adicional (si existe) */}
        {content.articles && content.articles.length > 0 && (
        ...
        {/* 10. CONTACT - Llamado a la acción final */}
